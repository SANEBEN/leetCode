package com.zzy;
// 本试卷唯一 ID: 5DB3308E6C3E1D9ACE10AA7A4BDEB712, 请勿修改本行内容
/* 注意, 这里不要写包名. 留空 */
...
class MetaJavaQuestionSheet {
    ...
    */
    ...
    /* qn13:
        ...
        E. SQL_2
    */
    public static String qn13 = "ACDE";
    /* qn14:
        public class Queue {
          private int size;
          private int[] data;
    ...
    */
    ...
    public static boolean searchMatrix(int[][] nums, int x) {
        for (int i = 0; i < nums.length; i++) {
            if (nums[i][i] == x) {
                ...
                if (i != 1 && nums[i][i] > x && nums[i - 1][i - 1] < x) {
    ...
    public static Set<String> showMap(Map<String, Object> map) {
        ...
        return showMap_Tool(map, new StringBuilder());
    }

    public static Set<String> showMap_Tool(Map<String, Object> map, StringBuilder title) {
        Set<String> set = new HashSet<>();
        map.forEach((key, value) -> {
            if (Object.class.isArray()) {
                Object[] arrays = (Object[]) value;
                for (int i = 0; i < arrays.length; i++) {
                    if (arrays[i] instanceof Map) {
                        if(title.length() == 0){
                            set.addAll(Objects.requireNonNull(showMap_Tool((Map<String, Object>) value, title.append(key).append("[")
                                    .append(i).append("]"))));
                        }else {
                            set.addAll(Objects.requireNonNull(showMap_Tool((Map<String, Object>) value, title.append(".").append(key).append("[")
                                    .append(i).append("]"))));
                        }
                    } else {
                        set.add(title.append(".").append(key).append("[")
                ...
                if(title.length() == 0){
                    set.addAll(Objects.requireNonNull(showMap_Tool((Map<String, Object>) value, title.append(key))));
                }else {
                    set.addAll(Objects.requireNonNull(showMap_Tool((Map<String, Object>) value, title.append(".").append(key))));
                }
            }
        });
        return set;
